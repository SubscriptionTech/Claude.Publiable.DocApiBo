...
1. **Pull the latest version of the Back-Office API specs** — Update the `shared/ProAbonoBO` submodule to the latest remote commit.
...
### Action 1 — Pull the latest version of the Back-Office API specs

Run the following command from the repository root:

```
git submodule update --remote shared/ProAbonoBO
```
...
The following source files in `shared/ProAbonoBO` are used when authoring the non-API-reference website pages:

| Source file | Used to author |
|---|---|
| `shared/ProAbonoBO/specs/index.md` | Introduction page |
| `shared/ProAbonoBO/specs/convention.md` | Authentication page, Getting Started page |
| `shared/ProAbonoBO/specs/actions.md` | Guides section (cross-resource flows) |
| `shared/ProAbonoBO/specs/resources/agent/` (all `.md` files) | Source reference for all API reference pages (generated) |

Follow these steps:

**4a. Identify what changed in the submodule.**
Run `git -C shared/ProAbonoBO log --oneline -20` to see recent commits. Then check for changes since the previous submodule commit using `git -C shared/ProAbonoBO diff HEAD~1 HEAD -- specs/` (adjust the path range to match the source files above if needed).

If the submodule was just updated (Action 1 was also checked), you can instead diff between the old and new commit:
- Before Action 1, the old commit was recorded in `git show HEAD:shared/ProAbonoBO` — use it here.
- After Action 1, the new commit is at `git rev-parse HEAD:shared/ProAbonoBO`.
